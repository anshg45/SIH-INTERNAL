...
def create_study(
    ...
    if pi_id is None and current_user.role == "admin":
        default_pi = db.query(User).filter(User.role == "pi").order_by(User.created_at.asc()).first()
        pi_id = default_pi.id if default_pi else None
    if pi_id is None:
        raise HTTPException(
